...
inline double _mm256_hsum_pd(const __m256d __A){
	return (__A[0] + __A[1] + __A[2] + __A[3]);
}

double compute_euclidean_distance_simd( const double *restrict query, const double *restrict train, int n)
{
	__m256d query_v, train_v, dist, temp;
	dist = _mm256_setzero_pd();
	for (int i = 0; i < n; i+=4) {
		query_v = _mm256_loadu_pd( &query[i] );
		train_v = _mm256_loadu_pd( &train[i] );
		temp  = _mm256_sub_pd( query_v, train_v );
		dist  = _mm256_fmadd_pd( temp, temp, dist );
	}
	return sqrt( _mm256_hsum_pd(dist) );
}
						...
						const double *restrict xtr, const int prob_dim, const int knn )
{
	...
	for (i = 0; i < TRAINELEMS; i++) {
		new_d = compute_euclidean_distance_simd(xquery, &xtr[i*prob_dim], prob_dim);
		if (new_d < max_d) {
			nn_x[max_i] = i;
			nn_d[max_i] = new_d;
...
int main(int argc, char *argv[])
{
	...
	for ( int i=0; i<QUERYELEMS; i++) {
		// Compute the knn and aproximate query value.
		double knn = find_knn_value( &xquery[i*PROBDIM], ytr, xtr, PROBDIM, NNBS );
		// Calculate Error
		sse += (yquery[i]-knn)*(yquery[i]-knn);
		err_sum +=  100.0*fabs((knn-yquery[i])/yquery[i]);
	} 
