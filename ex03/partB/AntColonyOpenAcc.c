...
void evaporatePherormoneACC(float * restrict pherormones, int pher_dim, float evaporateRate, const Antcell * restrict antsPos, int antCount){

	// Copy values get generated automatically correctly
	{
		#pragma acc parallel loop gang 
		for(int i = 0; i< pher_dim; i++){
			char flag=0;
			#pragma acc loop vector firstprivate(flag)
			for(int j=0; j < pher_dim; j++){	
				for(int k=0; k < antCount; k++){
					if ((antsPos[k].row == i) && (antsPos[k].col == j))
						flag = 1;
					// If the code is run on cpu if the condition is met
...
int main(int argc, char ** argv){
	...
	for ( int i=0; i < expCount; i++){
		...
		evaporatePherormoneACC(pherormones,N,0.3,antsPosNew,antCount);
	}
	t1 = omp_get_wtime();
