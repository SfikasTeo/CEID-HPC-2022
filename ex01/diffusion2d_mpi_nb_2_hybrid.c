...
void advance(Diffusion2D *D2D, int step , int checkpoint)
{
	...
	#pragma omp taskloop simd collapse(2) //num_tasks( omp_get_num_threads() )
...
int main(int argc, char *argv[])
{
	if (argc < 9)
	{
		...
		dt CheckPoint_ChunkSize Resume_Signal Hybric_Threads\n", argv[0]);
	...
	Diffusion2D system;
	char * file_checkpoint = "diffusion2d_mpi_nb_2_hybrid_checkpoint.bin";

	//possible issue when advance progresses much faster than write_density
	//will it affect the tmp_rho during two possible rewrites, when the first one
	//hasn't even completed?
	#pragma omp parallel 
	{
		#pragma omp master 
		{
			...
			if (rank == 0)
			{
				...
				sprintf(diagnostics_filename, "diagnostics_mpi_nb_2_hybrid_%d.dat", procs);
